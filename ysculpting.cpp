...
struct sculpt_params {
  ...
  // intersection
  ray3f            camera_ray       = {};
  scene_instance * shape_instance   = nullptr;
  bvh_tree         bvh_shape_tree   = {};
  bvh_intersection bvh_intersection = {};
...
sculpt_params *init_sculpt_tool(yocto::scene_model *ioscene,
    yocto::scene_camera *iocamera, std::string shapename,
    std::string texture_name, scene_material *material = nullptr) {
  sculpt_params *params = new sculpt_params();

  // create scene_shape and scene_instance
  scene_instance *shape_instance = add_instance(ioscene, "object");
  scene_shape *   shape          = add_shape(ioscene, "object");
  ...
  // set shape instance
  set_shape(shape_instance, shape);
  set_frame(shape_instance,
      frame3f{vec3f{1, 0, 0}, vec3f{0, 1, 0}, vec3f{0, 0, 1}, vec3f{0, 0, 0}});
  params->shape_instance = shape_instance;

  // save positions
  params->old_positions = shape_instance->shape->positions;

  // set/create scene_material
  if (material == nullptr) {
    material = add_material(ioscene, "clay");
    set_color(material, vec3f{0.78f, 0.31f, 0.23f});
    set_specular(material, 0.0f);
    set_material(shape_instance, material);
  } else {
    set_material(shape_instance, material);
  }
  shape_instance->shape->colors = vector<vec3f>(
      shape->positions.size(), vec3f{1.0f, 1.0f, 1.0f});
  // scene_texture* tex = add_texture(app->ioscene);
...
void view_pointer(scene_instance *instance, ogl_shape *glshape,
    bvh_intersection intersection, float radius, int definition,
    brush_type &type) {
  if (intersection.hit) {
...
int closest_vertex(scene_shape *shape, vec2f uv, int element) {
...
vector<pair<vec3f, vec3f>> stroke(
    sculpt_params *params, vec2f mouse_uv, ogl_camera *glcamera) {
  ...
  // (TODO) mettere questo ordine anche nella versione di github
  if (!params->lock) {
...
vector<pair<vec3f, vec3f>> symmetric_stroke(vector<pair<vec3f, vec3f>> &pairs,
    scene_instance *instance, bvh_tree &tree,
    vector<int> &symmetric_stroke_sampling, axes &axis) {
  // (TODO) pensare ad una possibile implementazione della simmetria non a
  // partire dall'origine del frame globale ma a partire dal frame della camera
  // (è meglio che il raggio parta da fuori il mesh piuttosto che da dentro)
...
vector<int> stroke_parameterization(yocto::geodesic_solver &solver,
  ...
  auto update = [&](int node, int neighbor, float new_distance) {
    ...
    // sembrano essere equivalenti i due metodi qui sotto per il calcolo di
    // weight
    float weight = length(positions[neighbor] - positions[node]) + flt_eps;
    weight       = 1.0f / weight;
    /*
    float weight = new_distance + flt_eps;
    weight       = 1.0f / weight;
    */
...
void apply_brush(scene_shape *shape, vector<vec3f> &positions,
    ogl_shape *glshape, bvh_tree &tree, hash_grid &grid) {
...
void brush(sculpt_params *params, ogl_shape *glshape,
    vector<pair<vec3f, vec3f>> &pairs) {
  ...
  // (TODO) rendere la soglia della saturazione settabile dall'utente, con
  // valore minimo = 1
...
void texture_brush(vector<int> &vertices, yocto::image<vec3f> &texture,
    vector<vec2f> &coords, sculpt_params *params, ogl_shape *glshape,
    vector<vec3f> positions, vector<vec3f> normals) {
  if (vertices.empty()) return;
  if (texture.empty()) return;

  auto scale_factor = 3.5f / params->radius;
  auto max_height   = 2.0f * gaussian_distribution(zero3f, zero3f, 0.7f,
...
void smooth(geodesic_solver &solver, vector<int> &stroke_sampling,
    vector<vec3f> &positions, sculpt_params *params, ogl_shape *glshape) {
  ...
  auto update       = [&](int node, int neighbor, float new_distance) {
    ...
    if (node != current_node) {
      ...
      //(TODO) aggiungere questo parametro strenght qui anche su github
      positions[current_node] += params->strength *
...
int main(int argc, const char *argv[]) {
  ...
  auto ioerror             = ""s;
  app->drawgl_prms.shading = ogl_shading_type::eyelight;
  ...
  callbacks.widgets_cb = [app, &sculpt_params](
    ...
    if (sculpt_params->type == brush_type::gaussian) {
      if (sculpt_params->strength < 0.9f || sculpt_params->strength > 1.5f)
      ...
      if (sculpt_params->strength > 1.0f) sculpt_params->strength = 0.5f;
      draw_slider(win, "radius", sculpt_params->radius, 0.1f, 0.8f);
